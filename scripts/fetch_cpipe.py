...
"""
Download the pre-built cpipe binary for the current platform from the
...
def _asset_name() -> tuple[str, bool]:
    ...
    if plat == "win32":
        if machine in ("amd64", "x86_64"):
            return "cpipe-x86_64-pc-windows-msvc.zip", True
        raise RuntimeError(f"No pre-built Windows binary for arch: {machine}")

    if plat == "darwin":
        if machine in ("arm64", "aarch64"):
            return "cpipe-aarch64-apple-darwin.tar.gz", False
    ...
    if machine in ("aarch64", "arm64"):
        return "cpipe-aarch64-unknown-linux-gnu.tar.gz", False
...
def _extract_binary(archive: Path, is_windows: bool, target_dir: Path) -> Path:
    """Extract the cpipe binary from the archive into target_dir.

    Release archives contain a single file named after the target triple
    (e.g. cpipe-x86_64-pc-windows-msvc.exe). We extract it and rename it
    to the canonical cpipe[.exe] name.
    """
    exe_name = "cpipe.exe" if is_windows else "cpipe"
    ...
    if archive.suffix == ".zip":
        ...
        with tarfile.open(archive, "r:gz") as tf:
            ...
            try:
                ...
                tf.extract(members[0], target_dir)
...
def main() -> None:
    ...
    print(f"context-pipe v{version}  |  platform: {sys.platform}/{platform.machine()}")
    print(f"Asset  : {asset}")
    print(f"Target : {dest}")

    base_url = (
        f"https://github.com/luismichio/context-pipe/releases/download/v{version}"
    )
    ...
    try:
        ...
        print("Done. Run `cpipe --help` to verify.")
    except Exception as e:
        print(f"Error fetching binary: {e}")
        print("Please build cpipe from source: cargo build --release --manifest-path crates/cpipe/Cargo.toml")
        sys.exit(1)
