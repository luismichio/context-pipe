...
def test_dynamic_pipe_runs_single_node():
    ...
    with patch("subprocess.Popen", return_value=_mock_popen("distilled output")):
        result, trace = run_dynamic_pipe(nodes, "raw input")
    assert result == "distilled output"
    assert len(trace) == 1
    assert trace[0]["node"] == "semantic-sift-cli"


def test_dynamic_pipe_runs_multiple_nodes():
    ...
    with patch("subprocess.Popen", side_effect=fake_popen):
        result, trace = run_dynamic_pipe(nodes, "raw input", allow_shell=True)
...
def test_dynamic_pipe_rejects_missing_cmd():
    """A node without 'cmd' raises ValueError."""
    nodes = [{"args": ["logs"]}]
    with pytest.raises(ValueError, match="missing required key 'cmd'"):
        run_dynamic_pipe(nodes, "some input")


def test_dynamic_pipe_rejects_shell_metacharacter_in_cmd():
...
def test_dynamic_pipe_node_crash_returns_error():
...
def test_dynamic_pipe_empty_nodes_returns_input():
    """Empty node list returns the original input unchanged."""
    result, trace = run_dynamic_pipe([], "original text")
    assert result == "original text"
    assert trace == []


def test_dynamic_pipe_trace_structure():
    """Trace entries have the expected keys."""
    nodes = [{"cmd": "semantic-sift-cli", "args": ["logs"]}]
    with patch("subprocess.Popen", return_value=_mock_popen("out")):
        _, trace = run_dynamic_pipe(nodes, "in")
    assert len(trace) == 1
    entry = trace[0]
...
def test_run_dynamic_pipe_allow_shell_end_to_end():
    ...
    with patch("subprocess.Popen", side_effect=fake_popen):
        result, trace = run_dynamic_pipe(nodes, "some log text", allow_shell=True)

    assert result == "distilled"
    assert len(trace) == 2


def test_run_dynamic_pipe_allow_shell_false_rejects_shell_cmd():
